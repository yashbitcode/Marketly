...
const segregateSellerOrders = asyncHandler(async (req, res, next) => {
...
const generateInvoice = asyncHandler(async (req, res) => {
    try {
        const { orderDocId, status } = req.order;

        if (status === "paid") {
            const order = await orderService.getOrderById({ orderDocId });
            const pdfBuffer = await createInvoice(order);

            const { fileId, name, url } = await imageKitService.upload(
                pdfBuffer,
            );
            ...
            const updatedOrder = await orderService.updateParentOrder(
                { _id: order._id },
                {
                    invoice: {
                        fileId,
