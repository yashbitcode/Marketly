...
const AuthApi = {
    ...
    changePassword: async (creds) =>
        await axiosClient.post(apiEndpoints.auth.changePassword, creds, {
            withCredentials: true,
        }),
    vendorLogin: async (creds) => await axiosClient.post(apiEndpoints.auth.vendorLogin, creds),
    superAdminLogin: async (creds) =>
        await axiosClient.post(apiEndpoints.auth.superAdminLogin, creds),
    register: async (creds) => await axiosClient.post(apiEndpoints.auth.register, creds),
    logout: async () => await axiosClient.post(apiEndpoints.auth.logout, { withCredentials: true }),
    ...
    resetPassword: async (token, payload) =>
        await axiosClient.post(apiEndpoints.auth.resetPassword + `/${token}`, payload),
    ...
    verifyEmailToken: async (sessionId, token) =>
        axiosClient.get(apiEndpoints.auth.verifyEmailCode + `/${sessionId}/${token}`),
};
