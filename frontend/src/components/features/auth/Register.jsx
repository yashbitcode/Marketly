        ...
        <Container className="font-inter flex px-4 gap-15 max-w-5xl py-7 m-auto justify-center items-center">
            ...
            <div className="flex flex-col items-center justify-center gap-5 w-full  text-dark max-w-120">
                ...
                <form className="w-full flex gap-4 flex-col" onSubmit={handleSubmit(onSubmit)}>
                    <Input label="Username" placeholder={"Username"} className="px-4 py-2" {...register("username")} error={errors?.username?.message} />
                    <Input label="Fullname" placeholder={"Fullname"} className="px-4 py-2" {...register("fullname")} error={errors?.fullname?.message} />
                    <Input label="Email Address" placeholder={"Email Address"} className="px-4 py-2" {...register("email")} error={errors?.email?.message} />
                    <Input label="Phone Number" placeholder={"Phone Number"} className="px-4 py-2" {...register("phoneNumber")} error={errors?.phoneNumber?.message} />
